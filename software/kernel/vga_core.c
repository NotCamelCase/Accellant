...
static __attribute__((constructor)) void vga_init(void)
{
    ...
    g_DisplayData.frontBuffer = malloc(SCREEN_WIDTH * SCREEN_HEIGHT * sizeof(DisplayPixel));
    g_DisplayData.backBuffer = malloc(SCREEN_WIDTH * SCREEN_HEIGHT * sizeof(DisplayPixel));
}
